...
document.addEventListener('DOMContentLoaded', function() {
    ...
    let video_param = parseInt(url.searchParams.get('epi')) || 0; // Provide a default value of 0 if video_param is undefined

    // Check if movie_data is an array and video_param is within a valid range
    if (Array.isArray(movie_data) && video_param >= 0 && video_param < movie_data.length) {
        videoEl.setAttribute('src', `http://localhost:8000/media/movies/${movie_data[video_param].fields.file}`);
    } else {
        console.error('Invalid movie data or video_param.');
    }
});


console.log('movie_data:', movie_data);
console.log('video_param:', video_param);
